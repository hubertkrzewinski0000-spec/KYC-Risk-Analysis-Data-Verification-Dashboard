...
├── style.css    — full design system (light + dark mode, chart styles)
├── data.js      — all mock KYC data with inline business-logic comments
├── app.js       — rendering functions, Chart.js charts, interaction logic
└── README.md    — this file
```
...
This makes the codebase readable both for engineers and for compliance stakeholders reviewing the project.
...
- **UI/UX for compliance tools** — tabular data, threshold tuning, severity-coded alerts, lazy chart initialisation
- **Accessibility** — semantic HTML, ARIA roles (`tablist`, `tab`, `tabpanel`), screen-reader labels on canvas elements

## Extending the Project

Possible next steps to demonstrate full-stack skills:

- Connect a real watchlist API (e.g. OpenSanctions, Dow Jones Risk & Compliance)
- Add a Node.js / Python backend with PostgreSQL for persistent case storage
- Implement CSV upload + real-time data cleaning pipeline
- Add animated chart transitions and drill-down on bar click
- Write Jest unit tests for the risk-scoring and threshold logic
- Deploy to GitHub Pages for a live demo link in the portfolio
